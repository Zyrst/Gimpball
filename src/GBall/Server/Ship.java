...
public class Ship extends GameEntity implements KeyListener {
	...
	*/
    ...
    public void readKeyInput(){
    	ConcurrentHashMap<ClientConnection, Client> clients = World.getInstance().getClientMap();
    	
    	for(Iterator itr = clients.entrySet().iterator(); itr.hasNext();){
    		Map.Entry<ClientConnection, Client> pair = (Map.Entry<ClientConnection, Client>)itr.next();
    		
    		if(m_id == pair.getValue().getShipID()){
    			boolean[] keys = pair.getValue().getKeys();
